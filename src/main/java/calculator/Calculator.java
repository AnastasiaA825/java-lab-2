 ...
 * Поддерживает базовые арифметические операции, математические функции и скобки.
 *
 * <p><b>Поддерживаемые операции:</b>
 * <ul>
 *   <li>Арифметические: +, -, *, /, ^ (степень), ! (факториал)</li>
 *   <li>Математические функции: sin, cos, tg, ctg, exp, log</li>
 *   <li>Скобки: () и модуль ||</li>
 *   <li>Переменные: буквенные обозначения (a, b, x, y и т.д.)</li>
 * </ul>
 *
 ...
 * double result1 = calculator1.getRes(); // 14.0
 *
 * Calculator calculator2 = new Calculator("sin(0) + cos(0)");
 * double result2 = calculator2.getRes(); // 1.0
 *
 * Calculator calculator3 = new Calculator("a + (b * 2)");
 * calculator3.setVariable("a", 5.0);
 * calculator3.setVariable("b", 2.0);
 * double result3 = calculator3.getRes(); // 9.0
 * }
 * </pre>
 *
 * @author ilabe
...
public class Calculator
{
    ...
    /**
     * Парсит математическое выражение и разбивает его на числа и операторы.
     *
     * @param expression выражение для парсинга
     * @param endingSymbol символ конца выражения (пробел, ')', '|')
    ...
    /**
     * Вычисляет значение выражения на основе распарсенных чисел и операторов.
     * Выполняет операции согласно приоритету.
     * Если в выражении есть неопределённые переменные, запрашивает их значения у пользователя.
     *
     * @throws IllegalArgumentException если в строке только операторы или встречается неизвестная переменная
     * @throws ArithmeticException если происходит деление на 0
     */
    ...
    /**
     * Преобразует выражение в строку с подставленными значениями переменных.
     * Заменяет все переменные на их числовые значения, исключая переменные внутри имен функций.
     *
     * @return строка выражения с подставленными значениями переменных
     */
    ...
    /**
     * Возвращает множество неопределённых переменных из выражения.
     * Неопределённой считается переменная, которая не является числом и отсутствует в variables.
     * Исключает специальные символы скобок и модуля.
     *
     * @return множество строк с именами неопределённых переменных
     */
    ...
    /**
     * Интерактивно запрашивает у пользователя значения для неопределенных переменных через консоль.
     * Для переменных с префиксом минуса автоматически применяется отрицание введенного значения.
     * Метод продолжает запрашивать ввод до тех пор, пока не будет получено корректное числовое значение.
     *
     * @param unresolvedVariables множество неопределенных переменных, для которых требуется запросить значения
     */
     ...
     * Математические функции имеют высший приоритет.
     * Для остальных операторов используется сравнение по operatorsPrecedence.
     *
     * @param op1 следующий оператор
     * @param op2 текущий оператор
     * @return true если op1 имеет более высокий приоритет чем op2
     */
     ...
     * @throws IllegalArgumentException если n отрицательное или не целое
     */
    ...
    /**
     * Внутренний конструктор для рекурсивного парсинга вложенных выражений.
     * Используется для обработки скобок и модуля.
     *
     * @param expression математическое выражение
     * @param endingSymbol символ конца выражения
     */
     ...
     * Например, setVariable("a", 1.0) установит:
     * - переменную "a" со значением 1.0
     * - переменную "-a" со значением -1.0
     *
